...
% parfor supported
%----------------------------------------------------
function  [im_out, rank_out] =denoise_optim_SVHT3(nim,b,step,Sig)
% to denoise using the optimal singular value hard thresholding.
...
[u, vals, v] = svd(Y, 'econ');
y= diag(vals);
tau_star= calcLambda(M/N)* sqrt(N)* sig;
y(y< tau_star) = 0;
X= u* diag(y)* v';
