...
sm= sm./max(sm(:));
% figure, myimagesc(sm(:,:,nz_center),mask)
...
sm = sm(:,:,nz_idx); % extract 9 slices

nz = size(dwi,3);

%% VST A vs B for noise estimation (use b1k only)
levels=1:2:10;% percent
ks= 5;
IM_R={};
parfor idx=1:length(levels)
    ...
    Sigma0(:,:,idx)= 0.01* level* sm(:,:,round(nz/2));
    Sigma1(:,:,idx)= std(squeeze(noisemap(:,:,round(nz/2),:)),0,3);
...
%
save -v7.3 sigEst3D_fastvarying_fullFOV_new_OddLevels Sigma_VST_A Sigma_VST_B ...
    Sigma_MPPCA Sigma0 Sigma1 levels sm mask IM_R dwi00

%%
load sigEst3D_fastvarying_fullFOV_new_OddLevels Sigma_VST_A Sigma_VST_B Sigma_MPPCA ...
...
%% VST A vs VST B for VST
 load sigEst3D_fastvarying_fullFOV_new_OddLevels Sigma_VST_B mask IM_R dwi00 levels
...
switch myconfig
    case 1
        VST_ABC='A';
        fn= 'IMVSTd_AvsB_A_3DNoise_OddLevels';
    case 2
        VST_ABC='B';
        fn= 'IMVSTd_AvsB_B_3DNoise_OddLevels';
    otherwise
end
...
%%
PSNR_A= load('IMVSTd_AvsB_A_3DNoise_OddLevels.mat','PSNR')
PSNR_B= load('IMVSTd_AvsB_B_3DNoise_OddLevels.mat','PSNR')
...
%%
sig_A= load('IMVSTd_AvsB_A_3DNoise_OddLevels.mat','Sigma_MPPCA')
sig_B= load('IMVSTd_AvsB_B_3DNoise_OddLevels.mat','Sigma_MPPCA')
soi=8;
