...
maxBoxDim=5;
figplot
%% Fig.4 PSNR of denoised images of single slice
clear all; clc; close all

nzToShow_idx = 45;
load data_2shell_brain_noisy IM_R dwi levels mask dwi00 bvals0
levels_all = levels;
...
IMs_denoised2 = zeros(S_all(1),S_all(2),S_all(3),10);
IMs_denoised2(:,:,:,nlevel_idx) = IMd_mppca;
load IMd_mppca_2shell_OddLevels IMd_mppca
nlevel_idx = [9 7 5 3 1];
IMs_denoised2(:,:,:,nlevel_idx) = IMd_mppca;

% load and resort mppca images
load IMVSTd_EUIVST_2shell IMVSTd_shrink_EUIVST
nlevel_idx = [10 8 6 4 2];
S_all = size(IMVSTd_shrink_EUIVST);
IMs_denoised1 = zeros(S_all(1),S_all(2),S_all(3),10);
IMs_denoised1(:,:,:,nlevel_idx) = IMVSTd_shrink_EUIVST;
load IMVSTd_EUIVST_2shell_OddLevels IMVSTd_shrink_EUIVST
nlevel_idx = [9 7 5 3 1];
IMs_denoised1(:,:,:,nlevel_idx) = IMVSTd_shrink_EUIVST;
...
sf=5;
fig_dwi_BrainSimuResults;
figure, position_plots(ims2,[1 4],[0 0.25],[],mask,'','y','gray',2)
...
% mppca
dwiName={'im_2shelll_mppca_level1',...
    'im_2shelll_mppca_level2',...
    'im_2shelll_mppca_level3',...
    'im_2shelll_mppca_level4',...
    'im_2shelll_mppca_level5',...
    'im_2shelll_mppca_level6',...
    'im_2shelll_mppca_level7',...
    'im_2shelll_mppca_level8',...
    'im_2shelll_mppca_level9',...
    'im_2shelll_mppca_level10'};
